...
[功能特色](#-核心亮點-key-features) • [系統架構](#-系統架構-architecture) • [資料庫設計](#-資料庫架構-database-schema) • [API 手冊](#-rest-api-端點手冊-api-reference) • [快速開始](#-快速安裝與部署-quick-start) • [生命週期算法](#-生命週期與評分算法-lifecycle-intelligence)
...
**Dora Radar** 不僅僅是 GitHub Trending 的靜態鏡像，而是一套專為 **AI 智能體架構師、開源探勘者與技術決策者** 量身打造的深度開源情報觀測系統。

系統每日定時對 GitHub 8 大技術榜單發起多執行緒平行爬取，結合自研 NLP 關鍵詞相關性引擎自動識別 AI Agent、提示詞工程、模型微調及開源工具鏈。透過內建的多通道翻譯機制，將第一手英文專案描述轉化為高精度的繁體中文；同時比對長期歷史資料庫，即時計算專案生命週期狀態（**今日首發新面孔**、**連續霸榜常客**、**星數爆發黑馬**），並自動聚合日、週、月跨維度趨勢。所有數據皆由高效能 FastAPI 後端驅動，並於現代美學的 Glassmorphic 淺色儀表板呈現，輔以 Telegram 定時推播，實現開源情報的全自主閉環監控。
...
- **跨頻道歸併與多重標籤**：自動識別跨榜專案（例如同時在 `All` 與 `Rust` 霸榜），保留更完整的技術元數據。

### 2. 🏷️ 智慧生命週期分析 (Smart Lifecycle Intelligence)
- ✨ **今日首發新面孔 (`is_new`)**：比對歷史 60+ 天數據庫，自動篩選出首次進入熱門榜的專案，避免長期被巨型專案佔據版面帶來的資訊疲勞。
- 🔥 **連續霸榜天數 (`consecutive_days`)**：動態追蹤專案連續維持熱門狀態的累積天數，識別長期具有高技術熱度的現象級項目。
...
- **純淨在地化視覺體驗**：前端卡片與專案歷程彈窗經過專屬排版優化，專注純繁體中文呈現，去除冗餘的次要英文重疊，提升資訊吸收效率。
...
- **頂級排版與圖標**：全站整合 Google Material Symbols Outlined 圖標，搭配現代幾何字體 Plus Jakarta Sans 與等寬科技字體 JetBrains Mono。
- **高對比彈窗專案歷程 (Solid White Modal)**：彈跳視窗採用 100% 不透明純白背景，杜絕背景透明干擾，並支援背景滾動鎖定與 ESC 鍵一鍵關閉。
...
- **每日 TOP 3 精選專案**：結合專案領域、發展潛力與智能體進化價值，產出深入的 Dora 智囊短評。
...
- **透明排程狀態列**：頂部導航即時呈現 Hermes 閘道排程狀態、上次巡檢成果、推播管道與下次巡檢倒數。
...
flowchart TD
    ...
    subgraph StorageLayer ["🗄️ 資料庫存儲層 (Supabase / PostgreSQL)"]
...
├── latest_insight.json          # 最新一期 Dora 智囊觀點結構化數據
...
本專案使用 PostgreSQL / Supabase 作為數據持久化存儲，包含三張核心數據表：
...
| `GET` | `/api/repo/{owner}/{repo}/history` | `limit` (預設 60) | 取得特定專案的歷史星數增長曲線與每日變化數據 |
...
## 💡 生命週期與評分算法 (Lifecycle Intelligence)

### 1. 相關性等級劃分 (Relevance Classification)
系統透過特徵字典對專案名稱與說明文字進行多維度加權評分：
- **🤖 Critical (極度相關)**：包含 `agent`、`autonomous`、`orchestrat`、`multi-agent`、`mcp`、`claude code`、`cursor`、`copilot`、`devin` 等關鍵字，專注智能體核心架構。
...
- **🛠 Low (一般通用)**：開發者常規工具、終端機 CLI、圖形編輯器、資料庫等一般開源項目。
...
### 4. 初始化資料庫 Schema
使用 PostgreSQL 用戶端（如 `psql` 或 Supabase SQL Editor）執行本專案提供的 [資料庫架構 Schema](#-資料庫架構-database-schema)。
...
| **資料持久化** | [Supabase](https://supabase.com/) / [PostgreSQL](https://www.postgresql.org/) | 結構化趨勢數據存儲、PostgREST 關聯查詢 |
| **前端架構** | 原生 HTML5 / CSS3 / ES6+ SPA | 無需繁瑣打包構建步驟，開箱即用 |
| **視覺美學** | Glassmorphism + Material Symbols | 明亮淺色毛玻璃質感、高對比彈窗、現代幾何字體 |
| **數據可視化** | [Chart.js 4.4+](https://www.chartjs.org/) | 語言圓餅圖、AI 長條圖、專案歷史增長曲線 |
...
1. Fork 本專案倉庫 (`https://github.com/Mi44Or30/dora-radar.git`)
